export default function VehicleCard({ vehicle, isOwnProfile = false, onEdit }) {
  if (!vehicle) {
    return isOwnProfile ? (
      <div style={{ backgroundColor: "var(--color-surface)", borderRadius: 12, padding: 16, marginBottom: 8, border: "1.5px dashed #E5E7EB" }}>
  ...
  return (
    <div style={{ backgroundColor: "var(--color-surface)", borderRadius: 12, padding: 16, marginBottom: 8, boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        <span style={{ fontSize: 15, fontWeight: 700, color: "var(--color-text)" }}>🚗 Vehicle</span>
        {isOwnProfile && (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px 16px" }}>
        {[
          ...
          <div key={k}>
            ...
            <div style={{ fontSize: 14, color: "var(--color-text)", fontWeight: 600 }}>{v}</div>
          </div>
        ))}
      </div>
    </div>
